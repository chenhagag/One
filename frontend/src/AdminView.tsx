              ...
              <div style={{ marginBottom: 16, padding: 12, border: "1px solid #e2e8f0", borderRadius: 8, background: "#fef9f0" }}>
                ...
                <div style={{ marginTop: 8 }}>
                  ...
                  {ratedMatches.map((m: any) => {
                    const thisRating = userId === m.user1_id ? m.user1_rating : m.user2_rating;
                    return (
                      <div key={`r-${m.id}`} style={{ padding: "6px 0", borderBottom: "1px solid #f1f5f9", fontSize: 12, direction: "rtl", display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
...
${footer}`)
                  ...
                  {
                    label: "עדכון לאחר תקלה",
                    subject: "עדכון לגבי תקלה שחווית ב-One",
                    html: wrap(`
<h2 style="color:#1B1464">היי${name ? " " + name : ""},</h2>
<p>זיהינו שבמהלך השימוש ${g("שלך", "שלך")} ב-One ${g("נתקלת", "נתקלת")} בתקלה שמנעה מחלק מהפעולות במערכת לעבוד או להישמר כראוי.</p>
<p>התקלות טופלו, וכעת אפשר להיכנס שוב ולנסות להמשיך בתהליך.</p>
<p>אנחנו מתנצלות על החוויה ועל הזמן ש${g("הקדשת", "הקדשת")} לניסיונות הקודמים. אם עדיין משהו לא עובד, אפשר לדווח לנו דרך המערכת — ואם גם הדיווח לא נשלח, אפשר פשוט לכתוב ישירות למייל התמיכה שלנו ונבדוק את זה באופן אישי.</p>
<p>תודה על הסבלנות,</p>
<p>צוות One</p>
<p style="margin-top:20px;font-size:13px;color:#888">📧 <a href="mailto:one-support@googlegroups.com" style="color:#7b5fa3">one-support@googlegroups.com</a></p>`)
                  },
                  {
...
${footer}`)
                  ...
                  {
                    label: "בקשת העלאת תמונה",
                    subject: `יש ${g("לך", "לך")} כיוון להתאמה ב-One 😊`,
                    html: wrap(`
<h2 style="color:#1B1464">היי ${name},</h2>
...
<p>בהצלחה,<br>צוות One</p>`)
                  ...
                  {
                    label: "כיוון להתאמה — דירוג",
                    subject: `יש ${g("לך", "לך")} כיוון להתאמה ב-One`,
                    html: wrap(`
<h2 style="color:#1B1464">היי ${name},</h2>
<p>יש לנו כיוון להתאמה ${g("עבורך", "עבורך")} שממתינה לבדיקתך.</p>
<p>${g("מוזמן", "מוזמנת")} להיכנס למערכת ולבדוק את ההתאמה.</p>
${btn("לבדיקת ההתאמה")}
${footer}`)
                  },
                ];
              ...
              {filtered.map(log => {
                ...
                return (
                  <tr key={log.id} style={{ borderBottom: "1px solid #eee", background: colors.bg, borderRight: sev !== "noise" ? `3px solid ${colors.border}` : "none" }}>
                    ...
                    <td style={{ padding: 6 }}>{log.user_id ? `${log.user_name || ""} (#${log.user_id})` : (() => { const m = log.route?.match(/\/users\/(\d+)/); return m ? `(#${m[1]})` : "-"; })()}</td>
...
function OutreachLogTab() {
  ...
  const tabs: { key: string; label: string; count: number }[] = [
    { key: "ratings", label: "דירוגים", count: newRatings },
    { key: "cancellations", label: "ביטולים", count: 0 },
    { key: "nudges", label: "בירורים", count: newNudges },
    { key: "questions", label: "שאלות", count: newQuestionAnswers },
    { key: "open_questions", label: "שאלות פתוחות", count: newOpenResponses },
    { key: "messages", label: "הודעות", count: 0 },
    { key: "whatsapp", label: "וואטסאפ", count: newWhatsapp },
  ];
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 20, flexWrap: "wrap" }}>
        {tabs.map(t => (
          <button key={t.key} onClick={() => setActiveSection(t.key)} style={{
            ...
            border: activeSection === t.key ? "2px solid #7b5fa3" : "1px solid #e5e7eb",
            borderRadius: 8, background: activeSection === t.key ? "#f5f0ff" : "#fff",
            color: activeSection === t.key ? "#7b5fa3" : "#374151",
            display: "inline-flex", alignItems: "center", gap: 5,
          }}>
            {t.label}
            {t.count > 0 && <span style={{ background: t.key === "whatsapp" ? "#25D366" : "#dc2626", color: "#fff", borderRadius: 20, padding: "1px 6px", fontSize: 10, fontWeight: 700 }}>{t.count}</span>}
          </button>
        ))}
      </div>
          ...
          <table style={s.table}>
            ...
            <tbody>
              {adminMessages.filter((m: any) => m.admin_message_type === "conversation").map((m: any) => {
                ...
                const isConversation = m.admin_message_type === "conversation";
                const responded = !!m.admin_message_responded_at;
                const responseSeen = m.admin_message_response_seen;
                return (
                  <tr key={m.user_id} style={responded && !responseSeen ? { background: "#fef3c7" } : undefined}>
                    ...
                    <td style={s.td}>
                      {responded
                        ? <span style={{ color: responseSeen ? "#16a34a" : "#dc2626", fontWeight: 700, fontSize: 12 }}>💬 הגיב/ה · {fmtDate(m.admin_message_responded_at)}{responseSeen ? " ✓" : ""}</span>
                        : dismissed
                    ...
                    <td style={s.td}>
                      <div style={{ display: "flex", gap: 4 }}>
                        {responded && !responseSeen && (
                          <button
                            style={{ padding: "3px 10px", fontSize: 11, border: "1px solid #16a34a", borderRadius: 4, cursor: "pointer", background: "#d1fae5", color: "#065f46", fontWeight: 600 }}
                            onClick={async () => {
                              await apiFetch(`/admin/users/${m.user_id}/mark-response-seen`, { method: "POST" });
                              setAdminMessages(prev => prev.map(x => x.user_id === m.user_id ? { ...x, admin_message_response_seen: true } : x));
                            }}
                          >ראיתי</button>
                        )}
                        <button
...
// ── Survey2 (WW) question metadata ──────────────────────────────

const SURVEY2_Q_LABELS: Record<string, string> = {
  "1": "איפה מחפשת היכרויות",
  "2": "מה חסר בדרכים הקיימות",
  "3": "עד כמה One הבינה אותך",
  "4": "חסר כאישה שמחפשת אישה",
  "5": "להרגיש יותר בבית ב-One",
  "6": "עזרה אחרי התאמה",
  "7": "התאמה עיוורת",
  "8": "להימנע מאקסיות",
  "9": "לשנות או להוסיף דבר אחד",
  "10": "איך הרגישה ההתאמה",
  "11": "מה הכי אוהבת ב-One",
  "12": "רעיונות להפצה",
  "13": "רוצה לעזור להפיץ",
};

const SURVEY2_Q_OPTIONS: Record<string, string[]> = {
  "1": ["אפליקציות היכרויות", "קבוצות / קהילות בפייסבוק או ברשתות אחרות", "אינסטגרם / רשתות חברתיות", "אירועים ומפגשים בקהילה", "דרך חברות / היכרויות מהחיים", "כרגע אני לא מחפשת"],
  "3": ["מאוד", "די הרבה", "באופן חלקי", "לא כל כך", "בכלל לא"],
  "7": ["כן, מעדיפה בלי תמונה", "אולי", "לא, חשוב לי תמונה", "לא בטוחה"],
  "8": ["מאוד חשובה", "די חשובה", "נחמד אבל לא קריטי", "לא חשובה", "לא רלוונטית"],
  "10": ["מאוד רלוונטית", "די רלוונטית", "דברים מדויקים + פערים", "לא מתאימה", "עוד לא קיבלתי"],
  "13": ["כן", "אולי", "כרגע לא"],
};

function Survey2AdminTab() {
  const [stats, setStats] = useState<any>(null);
  const [responses, setResponses] = useState<any[]>([]);
  const [users, setUsers] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    Promise.all([
      apiFetch("/admin/survey2/stats").then(r => r.json()),
      apiFetch("/admin/survey2/responses").then(r => r.json()),
      apiFetch("/admin/survey2/users").then(r => r.json()),
    ]).then(([st, resp, usr]) => {
      setStats(st);
      setResponses(resp);
      setUsers(usr);
  ...
  const handleSendEmails = async (testOnly?: string) => {
    ...
    setSendResult(null);
    try {
      const res = await apiFetch("/admin/survey2/send-emails", {
        method: "POST",
        body: JSON.stringify(testOnly ? { test_only_email: testOnly } : {}),
      });
      const data = await res.json();
      setSendResult(`נשלחו ${data.sent} מיילים${data.failed ? `, נכשלו ${data.failed}` : ""}`);
      const st = await apiFetch("/admin/survey2/stats").then(r => r.json());
      setStats(st);
    } catch {
      setSendResult("שגיאה בשליחה");
    }
  ...
  return (
    <div dir="rtl" style={{ padding: "16px 24px", maxWidth: 1000 }}>
      <h2 style={{ fontSize: 18, margin: "0 0 16px", color: "#1a1a2e" }}>סקר WW (נשים שמחפשות נשים)</h2>
      ...
      {viewMode === "stats" && responses.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {Object.entries(SURVEY2_Q_LABELS).map(([qId, label]) => {
            const options = SURVEY2_Q_OPTIONS[qId];
            if (!options) {
              const texts = responses.filter(r => r.responses[qId]?.text).map(r => ({ name: r.first_name, text: r.responses[qId].text, extra: r.responses[qId].otherText }));
              if (texts.length === 0) return null;
              return (
            ...
            <tbody>
              {users.map(u => (
                <tr key={u.id} style={{ borderBottom: "1px solid #f0f0f0" }}>
                  ...
                  <td style={s.td}>{u.first_name}</td>
                  <td style={s.td}>{u.email}</td>
                  <td style={s.td}>{u.survey2_email_sent_at ? new Date(u.survey2_email_sent_at).toLocaleDateString("he-IL") : "—"}</td>
                  <td style={s.td}>{u.survey2_banner_dismissed ? "כן" : "—"}</td>
                  <td style={s.td}>
                    <span style={{
                      padding: "2px 8px", borderRadius: 6, fontSize: 11, fontWeight: 600,
                      background: u.survey2_completed ? "#dcfce7" : u.survey2_updated_at ? "#fef3c7" : "#f3f4f6",
                      color: u.survey2_completed ? "#166534" : u.survey2_updated_at ? "#92400e" : "#888",
                    }}>
                      {u.survey2_completed ? "השלים" : u.survey2_updated_at ? "חלקי" : "לא התחיל"}
                    </span>
                  </td>
                </tr>
              ))}
              ...
              {responses.map(r => (
                ...
                >
                  ...
                  {selectedResponse?.user_id === r.user_id && (
                    <div style={{ marginTop: 12, borderTop: "1px solid #f0f0f0", paddingTop: 12 }}>
                      {Object.entries(r.responses).map(([qId, resp]: [string, any]) => {
                        const label = SURVEY2_Q_LABELS[qId] || `שאלה ${qId}`;
                        const options = SURVEY2_Q_OPTIONS[qId];
                        return (
                          <div key={qId} style={{ marginBottom: 10 }}>
                            <div style={{ fontSize: 12, color: "#999", fontWeight: 600 }}>{label}</div>
