      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 12, padding: "20px 24px", marginBottom: 16 }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <p style={{ fontSize: 13, color: "#555", lineHeight: 1.6, margin: 0 }}>
              ברגע שתמונת המצב שלך מספיק ברורה ומלאה, המאפיינים נכנסים למאגר ההתאמות הפעיל. האלגוריתם סורק ומחשב הצלבות והלימה בינך לבין שאר {isWW ? "המשתמשות" : "המשתמשים"} במערכת כדי למצוא את החיבור הנכון ביותר.
            </p>
          </div>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <p style={{ fontSize: 12, color: "#888", lineHeight: 1.5, margin: "6px 0 0" }}>
              איך זה עובד? ברגע שהאלגוריתם יזהה התאמה פסיכולוגית גבוהה, התמונות (לצד השם והגיל) יישלחו לאישור דיסקרטי של שני הצדדים. המשוב {isWW ? "שלכן" : "שלכם"} ידייק את ציון ההתאמה הסופי. בכל שלב אחר — התמונות והפרטים {isWW ? "שלכן" : "שלכם"} נותרים חסויים ונעולים לחלוטין.
            </p>
          </div>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: "#555", lineHeight: 1.6, margin: 0 }}>
              המערכת מציגה לך התאמה אחת בלבד — {isWW ? "האישה" : "האדם"} בעלת אחוז ההלימה הגבוה ביותר עבורך ברמה העמוקה ביותר. אנחנו לא {isWW ? "מתפשרות" : "מתפשרים"} על התאמות בינוניות ולכן {isWW ? "תקבלי" : "תקבלו"} התאמה רק כאשר {isWW ? "מישהי כזו תימצא" : "אדם כזה יימצא"}, זה עשוי לקחת זמן, במיוחד בשלבי ההתחלה כשאנחנו עוד {isWW ? "בונות" : "בונים"} את {isWW ? "מאגר המשתמשות" : "מאגר המשתמשים"} שלנו.
            </p>
            <p style={{ fontSize: 12, color: "#888", lineHeight: 1.5, margin: "6px 0 0" }}>
              ברגע שתמצא ההתאמה הטובה ביותר — {isWW ? "תקבלי" : "תקבלו"} הודעה חגיגית המלווה בהסבר על סיבות החיבור, {isWW ? "ותוכלי" : "ותוכלו"} להתחיל לדבר ולהכיר בצ'אט המשותף. בהצלחה!
            </p>
          </div>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: "#555", lineHeight: 1.6, margin: 0 }}>
              אנחנו {isWW ? "עושות" : "עושים"} את המיטב כדי שהחיבור הראשון יהיה ה-One {isWW ? "שלכן" : "שלכם"}, אבל אם {isWW ? "ניסיתן וגיליתן" : "ניסיתם וגיליתם"} שזה לא הלך — החיפוש לא נעצר. נחזיר {isWW ? "אתכן" : "אתכם"} מיד למאגר, אבל לא לפני שנערוך שיחת דיוק קצרה כדי להבין יחד מה עבד ומה פחות.
            </p>
            <p style={{ fontSize: 12, color: "#888", lineHeight: 1.5, margin: "6px 0 0" }}>
              כל פידבק {isWW ? "שלכן" : "שלכם"} וכל התאמה שלא צלחה הם דאטה קריטי שמלמד את ה-AI מה נכון {isWW ? "לכן" : "לכם"} לפעם הבאה. אנחנו מערכת לומדת — גם מכירה יותר {isWW ? "אתכן" : "אתכם"} ומה עובד {isWW ? "עבורכן" : "עבורכם"} וגם מייצרת לקחים רוחביים מהתאמות אחרות. ככל שהמאגר של One גדל — אנחנו {isWW ? "משתפרות ומדייקות" : "משתפרים ומדייקים"} את עצמנו.
            </p>
          </div>
        </div>
        ...
        <div style={{ textAlign: "center", maxWidth: 360 }}>
          ...
          {isPositive && (
            <p style={{ fontSize: 12, color: "#9ca3af", lineHeight: 1.7, margin: "0 0 28px", textAlign: "right" }}>
              חשוב לדעת: אישור ההתאמה בשלב הזה לא מבטיח שהיא אכן תצא לפועל. לפעמים התאמה אחרת מתקדמת קודם, {isWW ? "אחת המשתמשות כבר אינה זמינה" : "אחד המשתמשים כבר אינו זמין"}, או שהתהליך נעצר מסיבה אחרת. לכן, אם ההתאמה לא המשיכה — אין להסיק מכך בהכרח משהו לגבי האישור של הצד השני.
            </p>
          )}
          <button
            onClick={onBack}
...
export default function NewChat({ user, onBack, onNavigate, onUserUpdate, onLogout, adminViewing }: NewChatProps) {
  ...
  if (isWW) {
    channelNameMap["qa_search"] = "מה את מחפשת לי";
  }
              ...
              <>
                ...
                {chatListOpen && activeChannels.map(c => {
                  ...
                  return (
                    ...
                    >
                      <span style={{ flex: 1 }}>{channelNameMap[c.key] || c.name}</span>
        ...
        <div style={styles.header}>
          ...
          <span style={{ ...styles.headerTitle, flex: 1 }}>
            {screen === "home" ? <img src="/nameLogoTrans.png" alt="One" style={{ height: 16, objectFit: "contain", display: "block" }} /> :
             screen === "chat" ? (channelNameMap[channel] || "שיחה") :
                ...
                return (
                  <div style={styles.welcomeBlock}>
                    ...
                    <p style={styles.welcomeText}>
                      {isWW ? "העוזרת האישית" : "העוזר האישי"} שלך למציאת התאמה מדויקת ומשמעותית.
                    </p>
                    {!hideIntro && (
                      <>
                        <p style={styles.welcomeText}>
                ...
                <div style={{ padding: "0 24px 12px", maxWidth: 500, margin: "0 auto" }}>
                  <div style={{
                    ...
                    <p style={{ fontSize: 14, color: "#555", lineHeight: 1.7, margin: "0 0 16px" }}>
                      מצאנו {user.looking_for_gender === "woman" ? "מישהי" : user.looking_for_gender === "man" ? "מישהו" : "מישהו/י"} שנראה שיש {isWW ? "ביניכן" : "ביניכם"} חיבור מעניין ושווה בדיקה.
                      <br />כרטיס ההתאמה האישי {isWW ? "שלכן" : "שלכם"} מוכן, ואפשר לראות למה חשבנו שכדאי שתכירו.
                    </p>
                    {unreadMatchMessages > 0 && (
              ...
              <div className="nc-suggestions" style={{ ...styles.suggestions, ...(allChatsComplete ? { gap: 6 } : {}) }}>
                ...
                {(() => {
                  ...
                  const qaItems = inPool
                    ? [
                        { icon: "/icons/accurateMatch.png", text: isWW ? "מה בדיוק את מחפשת לי?" : "מה בדיוק אתה מחפש לי?", channel: "qa_search" },
                        { icon: "/icons/Question.png", text: "יש לי שאלה לגבי התהליך", channel: "qa_general" },
    ...
    <div style={{ flex: 1, overflowY: "auto", direction: "rtl" }}>
      <div className="nc-sub-screen-narrow" style={{ maxWidth: 400, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ ...sectionStyle, background: selfFrozen ? "#fef3c7" : "#f9fafb" }}>
          ...
          <p style={hintStyle}>
            {selfFrozen
              ? isWW ? "החיפוש מושהה. לא תקבלי התאמות חדשות. ניתן לחזור בכל עת." : "החיפוש מושהה. לא תקבלו התאמות חדשות. ניתן לחזור בכל עת."
              : isWW ? "אם תרצי הפסקה, אפשר להשהות את החיפוש. ההתאמות הקיימות יוקפאו ולא ייכנסו מועמדות חדשות." : "אם תרצו הפסקה, אפשר להשהות את החיפוש. ההתאמות הקיימות יוקפאו ולא ייכנסו מועמדים חדשים."}
          </p>
          {freezeMsg && (
