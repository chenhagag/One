...
### סקירת אוטומציה (GPT review — 24.9)
נסקרו קבצי האוטומציה, נקודות עיקריות:
- **כניסה למאגר**: צריך להבחין בין `in_matching_pool` (אוטומטי אחרי אנליזה) לבין `is_matchable` (pool + תמונה + look traits)
- **תובנות — closing_stage**: לוודא שהשאילתה דורשת >= 3 ולא >= 1
- **עדכון תובנות**: חסר שדה `insights_last_reviewed_at` למנוע בדיקות חוזרות על משתמשות שנבדקו ולא השתנו
- **סריקת שיחות**: לעבור מ"הודעות מהיום" ל-`last_scan_at` למנוע פספוס
- **תזכורות תמונה**: ממשיכים לשלוח עד מקסימום ניסיונות או העלאת תמונה — בלי בדיקת רלוונטיות התאמה ספציפית
- **תיאום תזכורות**: צריך cooldown גלובלי — שמשתמשת לא תקבל כמה תזכורות ביום אחד ממערכות שונות
- **⚠️ last_analysis_at גלובלי**: ניתוח חלקי (קבוצה אחת) מעדכן timestamp גלובלי ו"מסתיר" הודעות מקבוצות אחרות — צריך מעקב per-group
- **AI intent detection (עתידי)**: להעביר את תיוג השאלה לפרומפט עצמו — ה-AI יתייג כחלק מהתשובה, מה שנותן גם routing מדויק יותר וגם זיהוי "שינוי פרופיל משמעותי" לניתוח מיידי

### TODO לסשן הבא
- ⚠️ **מחיקת חשבון**: להחזיר hard delete למשתמשת (workaround פעיל)
- דף נחיתה + דפי הסבר משכנעים
- אוטומציה של pipeline (פערים מסקירה למעלה)
- בדיקת אפליקציית Android
- דיוק ציוני התאמה (17 תכונות סגנון)
- AI intent detection בפרומפט (להחליף regex — routing מדויק + trigger reanalysis חכם)
...
#### תיקון תמונות בדירוג התאמה (קריטי)
- **הבעיה**: תמונות נחתכו מהראש (`object-fit: cover` + `aspect-ratio: 4/5`) — רואים גוף בלי ראש
- **התיקון**: `object-fit: contain` — תמונה מלאה בלי חיתוך, רקע סגלגל ממלא שאר
- **ניווט**: נקודות (dots) מתחת לתמונה במקום פסי overlay דקים שלא נראו

#### בקשת תמונה במסך התאמה — שדרוג
- כפתור אחד פוצל לשניים: **"הודעה במערכת"** (admin_message) + **"שלח מייל"** (Resend)
- תיקון מגדר: "לך ולה" / "לך ולו" לפי `looking_for_gender` (לא מגדר המשתמש)
- עמודה חדשה `photo_request_sent_at` — אם כבר נשלחה בקשה, מוצג תאריך במקום כפתורים
- תוקן גם בתבנית מייל "בקשת העלאת תמונה" במסך המשתמש

#### מעקב שליחת הודעות מערכת
- עמודה חדשה `admin_message_sent_at` — מתעדכנת אוטומטית כששומרים הודעה
- הודעת מערכת במסך משתמש: "נשלחה: DD/MM/YY HH:MM" + "כניסה אחרונה" עם שעה + ✓ ירוק אם ראו
- שאלות מערכת: תאריכים מציגים שעות (HH:MM)

#### סקשן "הודעות מערכת למשתמשים" ביומן פניות
- טבלה חדשה: שם, הודעה (חתוכה 80 תווים), תאריך שליחה, נצפה (ירוק/כתום/אפור), כפתור "הסר"
- "הסר" מנקה את ההודעה מהמשתמש ומהרשימה
- Backfill: 29 הודעות ישנות קיבלו `admin_message_sent_at` מ-`updated_at`

### חקירות
- אור #278 תמונה עם "הופק באמצעות AI" — watermark של סמסונג S24 Ultra, לא מהמערכת
- ענבל #283 שגיאות 502 — Railway restart, לא בעיית קוד
- גילי #282 שגיאת 403 על system-question/answer — JWT פג תוקף, חד-פעמי

### סנכרון staging ↔ production
- שני הענפים זהים

### TODO לסשן הבא
- כפתור "ראיתי, תודה" למשתמשים — אפשרות לסמן הודעת מערכת כנקראה ולהעלים אותה
- להמשיך לעבור על פרומפט סגנון תכונה תכונה (17 תכונות נותרו)

---

## Previous Session: 2026-08-17 (Outreach Log + Critical Bug Fix + Rate Limit)

### ✅ עלה לפרודקשן

#### Rate Limit Fix
- `generalLimiter`: 300 → **1000** בקשות ל-15 דקות
- הבעיה: אדמין שגולש בין משתמשים חורג מהמגבלה → **כל** הבקשות נחסמות כולל טעינת תמונות של משתמשים אחרים
- הסימפטום: "Too many requests" + תמונות נעלמות (נראה כמו באג נפרד אבל אותה בעיה)

#### 🚨 תיקון באג קריטי — התאמות קודמות מזויפות
- **הבעיה**: מסך "התאמות קודמות" הציג **כל** match עם status `cancelled` — כולל התאמות שהאדמין ביטל מ-`potential_match` ומעולם לא נשלחו למשתמש
- **ההשפעה**: 5 משתמשים ראו התאמות שלא ידעו עליהן, 3 מהם השאירו פידבק על התאמות מזויפות
  - טל #168 (17 ביקורים!) — "צעיר מידי", "מבוגר מידי ולא לטעמי"
  - רונה דייויס #213 — "לא היייתי מודע עליה"
  - הדר #244 — ראתה 5 התאמות מזויפות
  - שני שוורץ #249 — ראתה 4 התאמות מזויפות
  - Nadav #23 — ראה 1 התאמה מזויפת
- **התיקון**: `match-history` + `has_past_matches` מסננים עכשיו `AND match_card_sent_at IS NOT NULL`
- **הודעות מערכת**: נשלחו ל-5 המשתמשים שנפגעו עם הסבר על הבאג

#### טאב "יומן פניות" חדש באדמין
- 3 סקשנים:
  - **דירוגי התאמות** — כל ההתאמות שנשלחו/דורגו (היסטורי מלא), עם דירוגי שני הצדדים, סטטוס צבעוני, פילטר הכל/ממתינים
  - **שאלות מערכת** — כל השאלות שנשלחו למשתמשים עם תשובות
  - **ביטולי התאמות** — מי ביטל, האם היתה התאמה אמיתית, פידבק מכל צד
- **עמודת "נצפה"** בדירוגים ובשאלות — בודק אם המשתמש נכנס למערכת מאז ששלחת

#### אינדיקטור "נצפה" במסך משתמש
- שאלות מערכת: "נכנס/ה ✓" / "לא נכנס/ה" ליד כל שאלה
- הודעת מערכת פעילה: תאריך כניסה אחרונה

#### כפתור "ראיתי" לדירוגי התאמות
- **מסך ניהול משתמשים (Pipeline)**: כפתור "ראיתי" → מסתיר דירוג מהרשימה
- **מסך משתמש (User Detail)**: אותו דבר — דירוגים שסומנו לא מוצגים
- **יומן פניות**: מציג הכל תמיד (היסטורי מלא, לא מושפע מ"ראיתי")
- Backend: `match-ratings/pending` מסנן `rating_admin_seen = FALSE`
...
## Previous Session: 2026-07-28 (Auto Pool Flow + Admin Cleanup + Bug Fixes)

### ✅ עלה לפרודקשן

#### שינוי תהליך כניסה אוטומטית למאגר
- Pipeline **לא שולח מייל** אוטומטית — מסמן `pool_email_pending = TRUE`
- Pipeline **לא מסמן** `analysis_completed = TRUE` — ממתין לבדיקת admin
- תובנות עדיין נוצרות אוטומטית, משתמש נכנס למאגר אוטומטית
- עמודה חדשה `pool_email_pending BOOLEAN DEFAULT FALSE` על `users`
- Action חדש `clear_pool_pending` ב-pipeline-action

#### סקשן אדמין חדש — "נכנסו אוטומטית למאגר"
- Stage חדש `auto_pool` בין "מוכנים למאגר" ל"במאגר"
- צבע סגול, פתוח כברירת מחדל (לא collapsed)
- כפתור "שלח מייל כניסה למאגר" עם נוסח מוכן
- כפתור "סיימתי טיפול" — מוריד pending ומעביר ל"במאגר" הרגיל

#### תיקון באג — מחיקת נתונים (reset-data)
- **נתיב שגוי**: Backend רשם `/api/users/:id/reset-data` עם prefix כפול
- **הרשאה שגויה**: `requireAdmin` במקום `requireUserAuth` — משתמש רגיל נחסם
- **חוסר רענון**: אחרי מחיקה מוצלחת הפרונטנד לא עשה reload — תוקן עם `window.location.reload()`

#### הסרת טאבים מיותרים מאדמין
- הוסרו: User Profiles, Enum Options, Config
- מחיקת dead code: ~215 שורות (3 קומפוננטות)

### משתמשי טסט חדשים בסטייג'ינג
- #178 טסט-א, #179 טסט-ב, #180 טסט-ג, #181 טסט-ד, #182 טסט-ה, #183 טסט-ו (testa1-6@test.com)

### סנכרון staging ↔ production
- שני הענפים זהים

### סשן ראשון מתאילנד
- כל השירותים נגישים: joinone.io, Railway staging, DB, GitHub
- Latency גבוה יותר (2-5 שניות) — משפיע רק על חוויית פיתוח, לא על משתמשים

### TODO לסשן הבא
- להמשיך לעבור על פרומפט סגנון תכונה תכונה (17 תכונות נותרו)
- לבדוק שה"ראיתי" לדירוגים עובד בפרוד

---

## Previous Session: 2026-07-27 (Style Prompt + Admin Match Management + Rescore)

### ✅ עלה לפרודקשן

#### כפתור "עדכון ציונים" (Rescore)
- פונקציה `rescoreExistingCandidates()` — מחשב מחדש ציונים לכל ה-candidate_matches
- מעדכן רק שדות ציונים (internal, external, final, כל הקטגוריות, profile_score)
- **לא נוגע** בסטטוס, הערות, או שום שדה אחר
- כפתור אדום "עדכון ציונים" באדמין

#### שיפור פרומפט סגנון
- **geekiness**: מדעי המוח/תכנות/אקדמיה ≠ גיקי. ציון 60+ רק עם אזכור מפורש של גיימינג/SF/פנטזיה/אנימה. בלי זה — מקסימום 35
- **mainstreamness**: סיגנלים נגד הזרם שוקלים יותר. תחביבים גנריים (ים, טיולים) לא מעידים על מיינסטרימיות. דוגמאות מפורשות: פסיכדליים, טקסים רוחניים, נוודות, פיקיות קיצונית
- הורץ ניתוח סגנון מחדש על כל 74 משתמשי המאגר בפרודקשן

#### משקלי Profile Score
- style: ×1 → **×2**
- attitudes: ×1 → **×1.5**
- schwartz: ×1 → **×1.5**
- סה"כ סגנון+עמדות+ערכים: 16% → 24% מהציון

#### מסך התאמה מפורט (Match Detail Modal)
- כפתור "צפה" בטבלת התאמות פותח מודל עם:
  - שני כרטיסי משתמש: תמונה, שם, גיל, עיר, מגדר, מחפש/ת, גובה, מעשן, ילדים, דת
  - פילטרי חיפוש: טווח גיל, טווח גובה, מיקום
  - פרופילים מחושבים: קוגניטיבי, רגשי-חברתי, רגשנות, תקשורת (weighted avg כמו UserDetail)
  - Big Five (כל 5, צבעוני), ערכים (top+bottom), MBTI טיפוס, אניאגרם טיפוס+כנף
  - סגנון בולט (כתום גבוה, אפור נמוך), עמדות
  - ציוני התאמה: פנימי, פרופיל פנימי, סופי, פרופיל + כל הקטגוריות
  - דירוגי משתמשים, שינוי סטטוס, כפתורי actions
  - הערות על ההתאמה

#### תקציר אדמין למשתמש
- textarea בראש דף משתמש (שדה `admin_notes` על users — כבר היה קיים ב-DB)
- רקע צהוב כשיש תוכן, כפתור שמור/ביטול

#### תמונות + תקציר בטבלת התאמות
- תמונה עגולה קטנה + admin_notes ליד כל משתמש בטבלת candidate matches
- Backend: subqueries לתמונה ראשונה + admin_notes

#### תמונה ראשית
- עמודת `is_primary` על `user_photos`
- כפתור "קבע ראשית" בגלריית תמונות באדמין (מסגרת סגולה + badge "ראשית")
- כל ה-queries מעדיפים `is_primary DESC` לפני `created_at ASC`
- Endpoint: `POST /admin/users/:id/photos/:photoId/set-primary`

#### "ראיתי" לדירוגי התאמות
- עמודת `rating_admin_seen` על `matches`
- כפתור "ראיתי" בדף משתמש ליד כל דירוג (סקשן "📊 דירוגי התאמות")

#### 📷 בקשת תמונה במסך התאמה
- כפתור "שלח הודעת תמונה" במודל ההתאמה — מופיע רק כשחסרה תמונה
- שולח `admin_message` מותאם מגדר + מצב הצד השני (יש/אין תמונה)

#### תבניות מייל חדשות
- "בקשת העלאת תמונה" — נוסח מותאם מגדר
- "כיוון להתאמה — דירוג" — לשלוח למשתמשים שצריכים לדרג

#### UI
- הסרת גלילה פנימית + sticky header מטבלת candidate matches

### סנכרון staging ↔ production
- staging היה מפגר 44 קומיטים אחרי main — סונכרן
- שני הענפים זהים עכשיו

### משתמשי טסט חדשים בסטייג'ינג
- #165 רון טסט, #166 טל טסט, #167 שני טסט, #168 נוי טסט, #169 רוני טסט, #170 דנה טסט, #171 אור טסט, #172 סתיו טסט

### TODO לסשן הבא
- להמשיך לעבור על פרומפט סגנון תכונה תכונה
- להזין look traits לדנה (#263) ורותם (#256) — אבדו בשחזור DB
- לבדוק שה"ראיתי" לדירוגים עובד בפרוד

---

## Previous Session: 2026-07-26 (Safety Hardening — Reset + Cascade)
