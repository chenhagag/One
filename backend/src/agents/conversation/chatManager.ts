  ...
  if (channel === "new_chat_cognitive") {
    ...
    if (lastAssistantMessage && message) {
      ...
      if (isReentry && lastAssistantMessage.includes("?")) {
        cognitiveExtra = `\n\n## שאלה שלא נענתה\nבפעם הקודמת שאלת שאלה שהמשתמש/ת לא הספיקה לענות עליה. ההודעה האחרונה שלך הייתה:\n"${lastAssistantMessage}"\n\nהזכר לה את השאלה בנעימות, למשל: "אגב, לפני כן שאלתי אותך שאלה שלא הספקנו לסיים — רוצה לענות עליה?" ואז חזור על השאלה.`;
      }
    }
  ...
  if (QA_CHANNELS.includes(channel)) {
    ...
    if (channel === "qa_about_me" || channel === "qa_insights") {
      ...
      if (userSummary) {
        ...
        parts.push("\n\n## סיכום מה שהמשתמש/ת שיתפה בשיחה\n" + summaryText);
      }
      ...
      if (excerpts.length > 0) {
        parts.push("\n\n## דברים שהמשתמש/ת אמרה בשיחות (ציטוטים לשימושך)");
      ...
      // Question banks for specific topics — female (default) and male variants
      const CALIBRATION_QUESTIONS_F: Record<string, string[]> = {
        mbti: [
          "כשאת חווה שבוע עמוס ומתיש נפשית, מה הדרך האפקטיבית ביותר שלך להטעין מצברים? לצאת לסביבה חברתית קלילה, או להתנתק לחלוטין לכמה שעות של שקט לבד?",
          ...
          "איך את מרגישה כשתסריט קבוע מראש משתנה ברגע האחרון? את מעדיפה שהלו\"ז והתוכניות שלך יהיו סגורים ומאורגנים מראש, או שאת פורחת דווקא באלתור וגמישות?",
        ],
        enneagram: [
          "כשאת מרגישה פגיעה או בלחץ חריג, מה התגובה האוטומטית שלך? את הופכת להישגית ומשימתית יותר, נסוגה פנימה לחשוב ולנתח, או מחפשת מישהי להישען עליה ולהרגיש בטוחה?",
          "אם היית צריכה לבחור את החשש העמוק ביותר שלך בחיים, מה הוא מבין השלושה: החשש לצאת פראיירית/שמישהו ישלוט בך, החשש להרגיש דחויה ולא אהובה, או החשש להיכשל ולהיתפס כלא מוצלחת?",
          "באיזו עמדה את מוצאת את עצמך הכי הרבה בחיים? המנהיגה שלוקחת אחריות ומנהלת, המטפלת שמקשיבה ועוזרת לכולם, או זו שמנסה לשמור על השקט, האופטימיות והשלום בין כולם?",
        ],
        values: [
          "אם היית צריכה לבחור בין עבודה סופר יציבה, בטוחה וצפויה לבין פרויקט עצמאי, הרפתקני ורווי סיכונים אבל עם חופש פעולה מוחלט — איפה הלב שלך נמצא?",
          "מה נותן לך תחושת סיפוק עמוקה יותר בסוף יום: לדעת שקידמת את המטרות האישיות שלך והגעת להישג משמעותי, או שתרמת למישהי אחרת או לחברה ושינית משהו לטובה?",
        ...
        bigfive: [
          "כשמשהו משתבש או מפתיע אותך לרעה ביומיום, כמה זמן לוקח לך 'להתאושש' רגשית? את נוטה להילחץ ולדאוג בקלות, או שאת נשארת קולית רוב הזמן וממשיכה הלאה?",
          "איך נראה ניהול המשימות שלך? את טיפוס מסודרת, מחושבת, שיורדת לפרטים הקטנים ומסיימת הכל בזמן, או שאת מעדיפה לעבוד בספונטניות ובבלגן מאורגן?",
          "עד כמה את מחפשת גירויים אינטלקטואליים, אמנותיים או חוויות יוצאות דופן בחיים שלך, לעומת העדפה ברורה למוכר, הנוח והפרקטי?",
        ],
        attachment: [
          "כשאת בקשר ומרגישה שבת/בן הזוג לוקחים פתאום קצת מרחק או פחות זמינים, מה המחשבה הראשונה שעולה לך בראש? דאגה שמשהו לא בסדר ביניכם, או שזה מרגיש לך טבעי לחלוטין ולא מטריד אותך?",
          "איך את מגיבה כשמישהי שרק הכרת מראה עניין חזק מאוד, נקשרת מהר ומבקשת אינטנסיביות? זה מרגיש לך מחבק ובטוח, או שזה מייצר אצלך רתיעה קלה ותחושת מחנק?",
          "עד כמה קל לך לסמוך על בנות זוג, להראות להן חולשה או להיות תלויה בהן רגשית כשקשה לך, מבלי לפחד שהן ינצלו את זה או ייעלמו?",
        ],
      };
      const CALIBRATION_QUESTIONS_M: Record<string, string[]> = {
        mbti: [
      ...
      // Select gender-appropriate questions (default: female)
      const CALIBRATION_QUESTIONS = gender === "man" ? CALIBRATION_QUESTIONS_M : CALIBRATION_QUESTIONS_F;
      ...
      const CLOSING_MSG = gender === "man"
        ? "תודה רבה על השיתוף, אני לוקח את כל מה שאמרת בחשבון ומריץ את הניתוח מחדש בהתאם למידע החדש. אם תרצה להוסיף עוד משהו בעתיד — תמיד אפשר לחזור לכאן."
        : "תודה רבה על השיתוף, אני לוקחת את כל מה שאמרת בחשבון ומריצה את הניתוח מחדש בהתאם למידע החדש. אם תרצי להוסיף עוד משהו בעתיד — תמיד אפשר לחזור לכאן.";
...
המשתמש/ת רוצה לדייק נושא ספציפי.
**ההודעה שלך חייבת להכיל אך ורק:**
1. משפט אחד קצר שמכיר בכך שרוצים לדייק
2. שאלה אחת: "מה בדיוק לא מרגיש לך מדויק? ספרי לי מה דעתך"
...
**אסור:** לנתח, להסביר, להציע חלופות, לשאול שאלות כיול, או לכתוב יותר מ-3 שורות.`;
        ...
        phaseInstruction = `\n\n## מבנה ההודעה — חובה לעקוב בדיוק
**ההודעה שלך חייבת להכיל אך ורק:**
1. תגובה קצרה (משפט אחד-שניים) לדעת המשתמש/ת — הכר במה שנאמר, בלי להתגונן
2. סיום עם: "אני יכולה לשאול אותך כמה שאלות קצרות שיעזרו לי לדייק את הניתוח, מה את אומרת?"
...
**אסור:** לנתח, להסביר תיאוריות, להציע טיפוסים חלופיים, לשאול שאלות כיול, או לכתוב יותר מ-4 שורות.`;
        ...
        phaseInstruction = `\n\n## מבנה ההודעה — חובה לעקוב בדיוק
**ההודעה שלך חייבת להכיל אך ורק:**
1. ${calibrationQuestionsAsked === 0 ? (gender === "man" ? '"מעולה, בוא נתחיל."' : '"מעולה, בואי נתחיל."') : "תגובה קצרה (משפט אחד) לתשובת המשתמש/ת"}
...
**אסור:** לנתח, לתת סיכום, להמשיך את השיחה, או לשאול שאלות נוספות.`;
        ...
        phaseInstruction = `\n\n## מבנה ההודעה — חובה לעקוב בדיוק
**ההודעה שלך חייבת להכיל אך ורק:**
1. תגובה קצרה (משפט אחד-שניים) למה שהמשתמש/ת אמרה
2. המשפט הבא (חובה להעתיק מילה במילה): "${CLOSING_MSG}"
...
**אסור:** לשאול שאלות נוספות או להמשיך את השיחה.`;
      ...
      if (qaExPattern.test(recentQaAboutMeText)) {
        ...
        qaExNote = qaExPattern.test(message)
          ? `\n- המשתמש/ת שואל/ת על אקסים או היכרות מוקדמת. הסבר שלפני קבלת התאמה סופית, שני הצדדים מקבלים את התמונות של הצד השני ויכולים לדחות מכל סיבה — כולל היכרות מוקדמת. בנוסף, אם רוצים — אפשר לכתוב כאן בצ'אט פרטים על אנשים ספציפיים, והמערכת תנסה לזהות ולהימנע מלהציע אותם.`
          : `\n- המשתמש/ת משתף/ת פרטים על מישהי שלא רוצה להתאמה איתה. **חובה** לכלול בתשובה: תודה על השיתוף, המידע ייכנס לניתוח והמערכת תנסה לזהות ולהימנע מלהציע אותן, **אבל לא ניתן להבטיח זיהוי מדויק ב-100%**.`;
      }

      parts.push(`\n\n## הנחיות${isDisagreeFlow ? " — המשתמש/ת חולקת על הניתוח" : ""}
אתה מנהל שיחה אישית עם המשתמש/ת על מה שלמדת עליה. יש לך למעלה את כל הנתונים שלה.

כללי תקשורת:
- לעולם אל תציין מספרים, ציונים או אחוזים. תרגם הכל לשפה תיאורית: "גבוה מאוד", "בינוני", "מאוזן", "נמוך יחסית".
- היה קצר וממוקד — שאל שאלה אחת טובה, תן למשתמש/ת לדבר. אל תכתוב פסקאות ארוכות.
- כשאתה מדבר על תכונות — השתמש בדוגמאות ממה שהמשתמש/ת אמרה בשיחות (יש לך ציטוטים למעלה).
- אם המשתמש/ת לא מסכימה — אל תתגונן. שאל שאלות חכמות כדי להבין למה.
- קרא לנוירוטיות "עוצמת תגובה רגשית". חשוב: זה לא מודד רגישות אלא נטייה לתנודות רגשיות ולחוויית רגשות בעוצמה גבוהה. אם משתמשת אומרת "אני רגישה" — הבהר בעדינות שהממד הזה מתאר את עוצמת התגובה הרגשית והתנודות, לא את עומק הרגישות או האמפתיה. תמיד הצג בכבוד ובאופן מעצים.${qaExNote}${phaseInstruction}`);
      ...
      if (exAcquaintancePattern.test(recentQaText)) {
        exAcquaintanceNote = exAcquaintancePattern.test(message)
          ? `\n- המשתמש/ת מודאגת מהתאמה עם מישהי שהיא כבר מכירה. הסבר שלפני קבלת התאמה סופית, שני הצדדים מקבלים את התמונות של הצד השני ויכולים לדחות מכל סיבה — כולל היכרות מוקדמת. בנוסף, אם רוצים — אפשר לכתוב כאן בצ'אט פרטים על אנשים ספציפיים (למשל שם של אקסית), והמערכת תנסה לזהות ולהימנע מלהציע אותן.`
          : `\n- המשתמש/ת משתפת פרטים על מישהי שלא רוצה להתאמה איתה. **חובה** לכלול בתשובה: תודה על השיתוף, המידע ייכנס לניתוח והמערכת תנסה לזהות ולהימנע מלהציע אותן, **אבל לא ניתן להבטיח זיהוי מדויק ב-100%**.`;
      }
...
המשתמש/ת שואלת שאלה על התהליך או המערכת.

כללי תקשורת — חובה:
- **ענה ישיר וקצר.** קודם תשובה ישירה (כן/לא/התשובה), אחר כך הרחבה קצרה אם צריך. לא יותר מ-3-4 משפטים בדרך כלל.
- **אל תחזור על מידע שכבר אמרת בשיחה.** אם כבר הסברת איך ההתאמה עובדת — אל תסביר שוב. המשתמש/ת זוכרת.
- **דבר בשפה טבעית, לא בפורמט FAQ.** אל תשתמש ברשימות ממוספרות. אל תכתוב כותרות בולדיות. פשוט דבר כמו בן אדם.
- **אל תסיים כל הודעה באותו משפט.** אל תכתוב "אני כאן בשבילך" או "אם יש שאלות נוספות". אם יש לך שאלה חזרה — שאל. אם לא — פשוט סיים.
- **השתמש במידע שניתן לך** (כולל ידע שנשלף על One, סטטוס המשתמש/ת, קונטקסט) כדי לענות בצורה מדויקת. אל תאמר "אני לא יודע" אם התשובה נמצאת שם.
- **כשמשתמשת שואלת על התאמה ספציפית** — גם אם אין לך פרטים על המועמדת, אתה כן יודע מה בדרך כלל אומרת התאמה ב-One: שנבדקו עשרות ממדים, שעברה סף, ומה הממדים. השתמש בזה כדי להסביר מה חיובי, בלי להמציא פרטים ספציפיים על המועמדת.
- ענה בשפה אנושית, אינטליגנטית ובגובה העיניים. לא שיווקית, לא מתנשאת.${exAcquaintanceNote}${adminConversationNote}`;
      ...
      if (channel === "qa_system" || channel === "qa_status" || channel === "qa_search" || channel === "qa_refine" || channel === "qa_match_feedback") {
        const richProfile = await formatRichProfileForChat(userId);
        if (richProfile.trim()) {
          contextBlock += "\n\n## נתוני הפרופיל האישיותי של המשתמש/ת (לשימוש בתשובה על 'מה את מחפשת לי')\n" + richProfile;
        }
      }
...
- אל תציין/י שקיבלת הנחיה או שזה ערוץ מיוחד. פשוט התעניין/י באופן טבעי.`;
  ...
  if (channel === "new_chat_taste") {
    ...
    if (isReentry && lastAssistantMessage && tasteUserMsgCount > 1) {
      reentryInstruction = `\n\n## חזרה לבדיקת טעם\nהמשתמש/ת חזרה לבדיקת הטעם. ההודעה האחרונה שלך הייתה:\n"${lastAssistantMessage}"\n\nאם הצגת פרופיל שהמשתמש/ת לא הספיקה להגיב עליו — הזכר לה בנעימות: "אגב, לפני כן הצגתי לך פרופיל — רוצה לחזור אליו?" וחזור על הפרופיל.`;
    }
    ...
    if (tasteUserMsgCount === 0) {
      ...
      phaseInstruction = `\n\n## שלב: פרופיל ראשון\nהמשתמש/ת אישרה שהיא מוכנה. הצג את פרופיל 1 מהרשימה למטה. העתק אותו בדיוק כמו שהוא ושאל: עד כמה היא הטעם שלך מ-1 עד 10?`;
    } else if (allProfilesDone) {
      // All profiles shown — summarize + close
      phaseInstruction = `\n\n## שלב: סיכום וסגירה — חובה לסגור עכשיו\nהצגת את כל הפרופילים. סכם בקצרה (2-3 משפטים) את הדפוס שעולה מהתגובות של המשתמש/ת — מה מושך אותה, מה פחות, איזה סגנון מדבר אליה.\nשאל את המשתמש/ת: "קלטתי נכון? יש משהו שהיית רוצה לדייק?"\nאם המשתמש/ת כבר אישרה או תיקנה — סיים עם: "תודה על הפתיחות, זה מאוד עוזר לי לדייק את ההתאמה."\n\n**חשוב מאוד:** אל תמציא פרופילים חדשים! אם המשתמש/ת מבקשת עוד — הסבר שמאגר הדוגמאות נגמר וזה בהחלט מספיק כדי לנתח את הטעם שלה. אם היא רוצה להוסיף משהו על הטעם שלה — היא מוזמנת לכתוב כאן והמערכת תתייחס.\nאל תציג עוד פרופילים. אל תמשיך את השיחה אחרי הסגירה.`;
    } else if (reachedMinimum && profilesShown === TASTE_MIN_PROFILES) {
      // Reached minimum — mid-summary + ask if they want more
      phaseInstruction = `\n\n## שלב: סיכום ביניים\nהצגת ${TASTE_MIN_PROFILES} פרופילים. סכם בקצרה (2-3 משפטים) את הדפוס שעולה מהתגובות — מה מושך אותה, מה פחות.\nשאל את המשתמש/ת: "קלטתי נכון? רוצה להמשיך לעוד כמה פרופילים או שמספיק?"`;
      ...
      if (wantsToStop) {
        phaseInstruction = `\n\n## שלב: סיכום וסגירה — חובה לסגור עכשיו\nהמשתמש/ת ביקשה לסיים. סכם בקצרה את הדפוס שעלה ושאל: "קלטתי נכון? יש משהו שהיית רוצה לדייק?"\nאם המשתמש/ת כבר אישרה — סיים עם: "תודה על הפתיחות, זה מאוד עוזר לי לדייק את ההתאמה."`;
      } else {
        // Continue with more profiles
        phaseInstruction = `\n\n## שלב: הצגת פרופיל\nהמשתמש/ת רוצה להמשיך. הצג את הפרופיל הבא מהרשימה (לפי הסדר — הפרופיל שעוד לא הוצג). העתק אותו בדיוק. אחרי הפרופיל שאל: עד כמה היא הטעם שלך מ-1 עד 10?\n\n**אל תחליט בעצמך שהפרופילים נגמרו. כשהם ייגמרו, תקבל הוראה מפורשת לסכם ולסגור.**`;
      }
    } else {
      // Show next profile from the list
      phaseInstruction = `\n\n## שלב: תגובה לפרופיל\n**חובה — אל תדלג על זה:** אם המשתמש/ת נתנה תגובה קצרה (מילה עד שני משפטים, כמו "חמודה", "בסדר", "לא משהו", ציון בלבד, "אהבתי") — חובה לדובב אותה קצת לפני שממשיכים. שאל מה אהבה ומה לא אהבה. למשל:\n- "מה ספציפית דיבר אלייך / לא דיבר אלייך?"\n- "היה משהו שבלט לך לטובה או לרעה?"\n- "מה אהבת? מה פחות?"\n\nזה חשוב כי בלי הפירוט לא נוכל לדייק את ההתאמה.\n\nרק אם התשובה כבר מפורטת (3+ משפטים עם הסבר ספציפי) — אפשר לעבור ישר לפרופיל הבא.\n\nכלל חשוב: אל תציג פרופיל חדש באותה הודעה עם שאלת הרחבה. או שאלה, או פרופיל — לא שניהם.\n\n**אל תחליט בעצמך שהפרופילים נגמרו. כשהם ייגמרו, תקבל הוראה מפורשת לסכם ולסגור.**`;
    }
  ...
  if (intent === "profile") {
    ...
    if (profileText.trim()) {
      ...
      if (userSummary) {
        const summaryText = formatSummaryForPrompt(userSummary);
        contextBlock = "\n\n" + PROFILE_CONTEXT + "\n\n## מה שלמדתי עליך מהשיחה (טרם בוצע ניתוח רשמי)\n" + summaryText + "\n\nהערה: זה מבוסס על מה שהמשתמש/ת שיתפה בשיחה. עדיין לא בוצע ניתוח אישיות מלא. שתף תובנות בצורה חמה ומעצימה, והדגש שככל שנמשיך לשוחח תוכלי ללמוד עליה עוד.";
      } else {
