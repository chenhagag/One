...
/**
 * Delete ALL data for a user across all FK-linked tables.
 * Order matters: child tables first, then parent tables, then users.
 */
async function deleteAllUserData(userId: number): Promise<void> {
  ...
  const tables: [string, string][] = [
    ...
    ["bug_reports", "DELETE FROM bug_reports WHERE user_id = $1"],
  ...
  for (const [table, sql] of tables) {
    try {
      ...
      console.error(`[deleteAllUserData] user=${userId} FAILED on ${table}:`, e.message);
      throw e;
    }
  }
}
...
app.delete("/admin/users/:id", async (req, res) => {
  ...
  // Delete all user data (shared with self-delete handler)
  await deleteAllUserData(userId);

  console.log(`[admin] Deleted user ${userId}`);
  return res.json({ deleted: true, user_id: userId });
});
  ...
  try {
    ...
    const result = users.map((u: any) => {
      ...
      return {
        ...
        desired_age_min: u.desired_age_min ?? null,
        desired_age_max: u.desired_age_max ?? null,
        age_flexibility: u.age_flexibility || null,
        desired_height_min: u.desired_height_min ?? null,
        desired_height_max: u.desired_height_max ?? null,
        height_flexibility: u.height_flexibility || null,
  ...
  try {
    ...
    if (!displayMessage) {
      ...
      if (isCouple && chatCount >= 1) {
        ...
        displayMessage = "מאחר וסימנת שאינך מעוניינת לקבל עדכונים במייל — נציג לך הודעות חשובות במסך כאן. כדאי להיכנס מדי פעם לבדוק אם יש התאמה שלא תתפספס :) את מוזמנת גם לשנות את ההגדרה במסך ההגדרות.";
      }
    }
    ...
    // Survey status — old survey disabled (removed from home screen)
    ...
    const survey2BannerDismissed = await pgQueryOne<{ survey2_banner_dismissed: boolean }>(
      "SELECT COALESCE(survey2_banner_dismissed, FALSE) as survey2_banner_dismissed FROM users WHERE id = $1", [userId]
    );
    ...
    return res.json({
      ...
      show_survey2_banner: showSurvey2Banner,
      survey2_partial: survey2Row && !survey2Row.completed ? true : false,
...
// ── Survey 2 (WW-focused survey) ──────────────────────────────

// GET /api/survey2/my-response — Check if user already completed or has partial
app.get("/survey2/my-response", requireUserAuth, async (req, res) => {
  const userId = (req as any).userId;
  try {
    const row = await pgQueryOne<{ responses: any; completed: boolean }>(
      "SELECT responses, completed FROM survey2_responses WHERE user_id = $1", [userId]
    );
    return res.json({ responses: row?.responses || {}, completed: row?.completed || false });
  } catch (err: any) {
    return res.status(500).json({ error: err.message });
...
// POST /api/survey2/response — Save (partial or complete) survey2 response
app.post("/survey2/response", requireUserAuth, async (req, res) => {
  const userId = (req as any).userId;
  const { responses, completed } = req.body;
  if (!responses || typeof responses !== "object") return res.status(400).json({ error: "responses object required" });
  ...
  try {
    ...
    await pgQueryAll(
      `INSERT INTO survey2_responses (user_id, responses, completed, updated_at)
       VALUES ($1, $2, $3, NOW())
       ON CONFLICT (user_id) DO UPDATE SET responses = $2, completed = $3, updated_at = NOW()`,
      [userId, JSON.stringify(responses), completed || false]
    );
...
// POST /api/survey2/dismiss-banner — Dismiss survey2 banner on home screen
app.post("/survey2/dismiss-banner", requireUserAuth, async (req, res) => {
  const userId = (req as any).userId;
  try {
    await pgQueryAll("UPDATE users SET survey2_banner_dismissed = TRUE WHERE id = $1", [userId]);
    return res.json({ ok: true });
  } catch (err: any) {
    return res.status(500).json({ error: err.message });
  }
...
// GET /admin/survey2/stats — Survey2 overview stats
app.get("/admin/survey2/stats", requireAdmin, async (_req, res) => {
  try {
    const total = await pgQueryOne<{ c: number }>(
      `SELECT COUNT(*)::int AS c FROM users WHERE (test_user_type IS NULL OR test_user_type != 'Couple Tester')
       AND email IS NOT NULL AND email != ''
       AND gender != 'man' AND looking_for_gender IS DISTINCT FROM 'man'`,
      []
    );
    const emailed = await pgQueryOne<{ c: number }>(
      "SELECT COUNT(*)::int AS c FROM users WHERE survey2_email_sent_at IS NOT NULL", []
    );
    const completed = await pgQueryOne<{ c: number }>(
      "SELECT COUNT(*)::int AS c FROM survey2_responses WHERE completed = TRUE", []
    );
    const partial = await pgQueryOne<{ c: number }>(
      "SELECT COUNT(*)::int AS c FROM survey2_responses WHERE completed = FALSE", []
    );
    const dismissed = await pgQueryOne<{ c: number }>(
      "SELECT COUNT(*)::int AS c FROM users WHERE survey2_banner_dismissed = TRUE", []
    );
    return res.json({
      total_eligible: total?.c ?? 0,
      emails_sent: emailed?.c ?? 0,
...
// GET /admin/survey2/responses — All survey2 responses with user info
app.get("/admin/survey2/responses", requireAdmin, async (_req, res) => {
  try {
    const rows = await pgQueryAll<any>(
      `SELECT sr.user_id, sr.responses, sr.completed, sr.created_at, sr.updated_at,
              u.first_name, u.email, u.gender, u.survey2_email_sent_at
       FROM survey2_responses sr
       JOIN users u ON u.id = sr.user_id
       ORDER BY sr.updated_at DESC`
    );
    return res.json(rows);
...
// GET /admin/survey2/users — All eligible WW users with survey2 status
app.get("/admin/survey2/users", requireAdmin, async (_req, res) => {
  try {
    const rows = await pgQueryAll<any>(
      `SELECT u.id, u.first_name, u.email, u.gender, u.survey2_email_sent_at, u.survey2_banner_dismissed,
              sr.completed AS survey_completed, sr.updated_at AS survey_updated_at
       FROM users u
       LEFT JOIN survey2_responses sr ON sr.user_id = u.id
       WHERE (u.test_user_type IS NULL OR u.test_user_type != 'Couple Tester')
         AND u.email IS NOT NULL AND u.email != ''
         AND u.gender != 'man' AND u.looking_for_gender IS DISTINCT FROM 'man'
       ORDER BY u.id`
    );
    return res.json(rows);
  } catch (err: any) {
...
// POST /admin/survey2/send-emails — Send survey2 email to eligible WW users
app.post("/admin/survey2/send-emails", requireAdmin, async (req, res) => {
  if (!resend) return res.status(500).json({ error: "Email service not configured (missing RESEND_API_KEY)" });

  const { test_only_email } = req.body;

  try {
    let query = `SELECT id, first_name, email FROM users
                 WHERE (test_user_type IS NULL OR test_user_type != 'Couple Tester')
                   AND email IS NOT NULL AND email != ''
                   AND survey2_email_sent_at IS NULL
                   AND gender != 'man' AND looking_for_gender IS DISTINCT FROM 'man'`;
    const params: any[] = [];
    ...
    <a href="https://www.instagram.com/joinone.oneapp" style="margin:0 8px;display:inline-block"><img src="https://cdn-icons-png.flaticon.com/24/2111/2111463.png" alt="Instagram" style="width:28px;height:28px" /></a>
      ...
      try {
        await resend.emails.send({
          from: "One <noreply@joinone.io>",
          to: u.email,
          subject: "One - היכרויות חכמות בין נשים",
          html: wrapEmailFooter(html),
        });
        await pgQueryAll("UPDATE users SET survey2_email_sent_at = NOW() WHERE id = $1", [u.id]);
        await pgQueryAll("INSERT INTO email_log (user_id, subject, email_type) VALUES ($1, $2, $3)",
          [u.id, "One - היכרויות חכמות בין נשים", "survey2"]);
        sent++;
      } catch (emailErr: any) {
        console.error(`[survey2-email] Failed to send to ${u.email}:`, emailErr.message);
        failed++;
      }
    }
